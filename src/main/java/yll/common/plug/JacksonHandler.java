...
public class JacksonHandler implements JsonHandler {
    ...
    private final MyObjectMapper mapper = MyObjectMapper.INSTANCE;
    ...
    public Mapx toMap(String json) {
        try {
            return TreeNodeConverts.toMapx(mapper.readTree(json));
        } catch (Exception e) {
    ...
    public Listx toList(String json) {
        try {
            return TreeNodeConverts.toListx(mapper.readTree(json));
        } catch (Exception e) {
            logger.error(e.toString(), e);
            return null;
        }
